#!/usr/bin/env bash
#
# Redeploys fewcoco-core from scratch: fresh clone, tagged and pushed at the
# as-cloned commit (if -version is given), static config restored from
# deploy-config/, .env built from the repo's own .env.example template
# merged with deploy-config's deploy.env/secret.env overrides, old stack
# torn down, new stack built and health-checked, and (on success, if
# -version was given) a release-notes.md entry appended. Lives in
# deploy-config/ so it survives repeated clone/delete cycles of the app
# repo itself.
#
# Unlike tools-myjoyfulco-com's deploy.sh (v1), this stack is a single
# service (api) — fewcoco_core_db lives inside v1's own Postgres container,
# reached over that container's compose network (see docker-compose.yml's
# external "esa" network and deploy.env's DB_HOST=db). Run bootstrap-db.sh
# once, before the first deploy ever run against a given database, to
# create fewcoco_core_db/fewcoco_app and copy v1's users/invite_codes/
# etsy_oauth_tokens rows — this script does not do that.
#
...
#
# Usage: ./deploy.sh [-version <vX.Y.Z>] [-revert-to <tagname>]
...
#
# --wipe-db is not offered here — there is no db service or volume in this
# stack to wipe. Use bootstrap-db.sh (drops/recreates fewcoco_core_db
# itself) for that instead.
...
# ---- argument parsing ----
usage() {
  echo "Usage: $0 [-version <vX.Y.Z>] [-revert-to <tagname>]" >&2
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --wipe-db)
      usage
      echo "--wipe-db is not supported here — this stack has no db service or volume. fewcoco_core_db lives in v1's Postgres container; use bootstrap-db.sh to reset it instead." >&2
      exit 1
      ;;
    -version)
      if [[ $# -lt 2 ]]; then
...
log "Starting redeploy (branch=$BRANCH, version=${VERSION:-none}, revert-to=${REVERT_TO:-none})"
...
log "Step 3/7: copying Dockerfile, docker-compose.yml from deploy-config"
...
log "Step 4/7: building .env (repo .env.example as base; secret values never printed)"
ENV_OUT="$APP_DIR/.env"

# Unlike v1, this repo gitignores .env and tracks only .env.example — start
# from that template rather than requiring a committed .env.
require_file "$APP_DIR/.env.example"
cp "$APP_DIR/.env.example" "$ENV_OUT"

# The repo's own .env.example (committed template) is the source of truth
# for which keys exist. deploy.env and secret.env only ever overwrite
# values for keys already present there — neither can introduce a new key.
...
APP_UP=false
if echo "$COMPOSE_PS_OUTPUT" | grep -qE '^fewcoco-core-api-1.*Up'; then
  APP_UP=true
fi

if [[ "$APP_UP" != true ]]; then
  fail "Container is not Up (api=$APP_UP)"
fi
log "Container is Up"
